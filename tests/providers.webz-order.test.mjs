...
test('newsapi-ai provider returns items; domains handled post-fetch', async () => {
  ...
  assert.ok(res.request, 'should include request metadata')
  assert.equal(res.request.variant, 'as-is', 'should note variant label')
  assert.equal(res.request.method, 'POST', 'should record HTTP method')
  assert.equal(res.request.body.apiKey, '[redacted]', 'should redact API key in request body')
  assert.equal(res.request.body.articlesCount, 5, 'should expose sanitized request payload details')
})
