 ...
 *  - Write-through on positive cache sets (no negative entries persisted).
 *  - TTL alignment: reuse fresh TTL for now; can evolve to longer L2 retention later.
 */
...
// Simple per-instance fallback provider (helps testing the integration logic)
...
class UpstashProvider implements L2Provider {
  ...
  constructor(base: string, token: string) {
    this.base = base.replace(/\/$/, '') // trim trailing slash
    this.token = token
  }
  available() {
  ...
  async get<T>(key: string): Promise<T | null> {
    try {
      ...
      // Values were JSON.stringified on write; attempt parse
      try {
  ...
  async set<T>(key: string, value: T, ttlSeconds: number) {
    try {
      ...
      const ttl = Math.max(30, Math.min(86400, ttlSeconds || 60)) // up to 24h cap
