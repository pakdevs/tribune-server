// Phase 7: Durable hourly metrics rollups.
// This module periodically snapshots in-process metrics (_metrics.ts) and merges them
// into an hour bucket stored in KV (if available) or kept in global memory fallback.
// Key format: metrics:rollup:YYYYMMDDHH
// Structure (JSON): {
//   hour: 'YYYYMMDDHH',
//   startedAt: epoch_ms_first_write,
//   updatedAt: epoch_ms_last_merge,
//   httpStatus: { 200, 304, 4xx, 5xx, 429 },
//   httpLatency: { buckets, sumMs, count },
//   upstreamLatency: { buckets, sumMs, count },
//   upstream: { ok, error, timeout },
//   etag304: number
// }
// Retention: configurable via env ROLLUP_RETENTION_HOURS (default 72). Older keys purged best-effort.
// Flush interval: env ROLLUP_FLUSH_INTERVAL_MS (default 300000 = 5m).
...
export function __test_only_injectHour(hour, docPartial) {
  ...
  globalThis.__metricsRollups.set(hour, merged)
}
...
export function __test_only_forceRetentionPurge() {
  ...
  const now = Date.now()
  for (const [hour] of globalThis.__metricsRollups.entries()) {
    const ts = Date.UTC(
      Number(hour.slice(0, 4)),
      Number(hour.slice(4, 6)) - 1,
      Number(hour.slice(6, 8)),
    ...
    if (now - ts > maxAge) globalThis.__metricsRollups.delete(hour)
  }
}
