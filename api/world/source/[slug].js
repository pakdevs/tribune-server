...
export default async function handler(req, res) {
  ...
  // world defaults to country=us for some providers that need it
  ...
  try {
    ...
    if (!flight) {
      flight = setInFlight(
        flightKey,
        (async () => {
          // Order providers: prefer gnews first if present
          ...
          for (const p of ordered) {
            ...
            try {
              ...
              if (!Array.isArray(items) || !items.length) {
                attemptsDetail.push(`${p.type}(empty)`) // continue
                continue
              }
              // Normalize and filter to the target source
              ...
              attemptsDetail.push(`${p.type}(no-source-match)`) // had items, none matched target
            } catch (e) {
              attemptsDetail.push(`${p.type}(err)`) // continue to next provider
            }
          }
