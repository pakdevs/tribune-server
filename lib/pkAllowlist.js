...
// Matching behavior: any subdomain of an entry is also considered allowed
// (e.g., images.dawn.com matches entry "dawn.com").
const SEED = [
  'dawn.com',
  'geo.tv',
  'samaa.tv',
...
export function isHostInAllowlist(host, allowlist) {
  ...
  if (!h) return false
  const list = Array.isArray(allowlist) ? allowlist : SEED
  // Exact match or subdomain match (e.g., foo.bar.com matches bar.com)
