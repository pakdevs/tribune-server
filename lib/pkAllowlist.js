...
// Explicit toggle for using Upstash for allowlist reads/writes.
// Default: off to avoid unintended Upstash command usage.
const ALLOWLIST_USE_UPSTASH = String(process.env.ALLOWLIST_USE_UPSTASH || '0') === '1'
...
function getUpstashClient() {
  if (!ALLOWLIST_USE_UPSTASH) return null
...
async function readAllowlistFromStore() {
  // Prefer Vercel KV first (does not affect Upstash usage)
  try {
    const val = await kv.get('pk:allowlist')
    if (Array.isArray(val)) return { list: val, source: 'kv' }
  } catch {}
  // Optionally try Upstash if explicitly enabled and creds present
